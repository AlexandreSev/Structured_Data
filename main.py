...
def main(folder=None, n_model=1, nb_epoch=100, save=True, warmstart=False, 
	...
	list_file, list_target = preprocess_data(data, n_model=n_model)

	with tf.Session() as sess:

		if n_model == 1:
			model = model1.first_model()
			training_target = one_hot(list_target)
		else:
			model = model2.second_model()
			training_target = None

		sess.run(tf.global_variables_initializer())
		model.train(list_file, list_target, sess, nb_epoch, save, warmstart, 
					weights_path, save_path, training_target)


if __name__ == "__main__":
	main(folder=1, n_model=1)
